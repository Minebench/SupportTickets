...
public class SQLController implements DatabaseController {
        ...
        if (connection != null) {
            try {
                PreparedStatement statement = connection.prepareStatement(
                        "Select *, (Select uuid from " + SupportTicketsConfig.getPlayerTable() + " where player_id = t.closed_player_id) as uuidPlayerClosed "
                                + " from " + SupportTicketsConfig.getTicketTable() + " t"
                                + " left join " + SupportTicketsConfig.getPlayerTable() + " p on p.player_id = t.player_id"
                                + " where t.ticket_id = ?");
    ...
    public int saveTicket(Ticket ticket) {
        ...
        try {
            if (connection != null) {
                ...
                preparedStatement = connection.prepareStatement("INSERT INTO " + SupportTicketsConfig.getTicketTable() +
                        " (player_id, message, time_stamp, status, server, world, loc_X, loc_Y, loc_Z, yaw, pitch) VALUES " +
                        "((Select player_id from " + SupportTicketsConfig.getPlayerTable() + " where uuid = ?), " +
            ...
            public void run() {
                ...
                try {
                    if (connection != null) {
                        ...
                        PreparedStatement preparedStatement = connection.prepareStatement(
                                "UPDATE " + SupportTicketsConfig.getTicketTable() + " SET assigned = ?, assigned_time_stamp = ?, status = ? where ticket_id = ?");
            ...
            public void run() {
                ...
                try {
                    if (connection != null) {
                        // update the target, the time_stamp and the status
                        PreparedStatement preparedStatement = connection.prepareStatement(
                                "UPDATE " + SupportTicketsConfig.getTicketTable() + " SET assigned = null, assigned_time_stamp = null, status = ? where ticket_id = ?");
                ...
                try {
                    if (connection != null) {
                        ...
                        preparedStatement = connection.prepareStatement(
                                "UPDATE " + SupportTicketsConfig.getTicketTable() + " SET " +
                                        "closed_player_id = (Select player_id from " + SupportTicketsConfig.getPlayerTable() + " where uuid = ?), " +
                                        "closed_time_stamp = ?, " +
                                        "closed_reason = ?, " +
        ...
        SupportTickets.getInstance().getServer().getScheduler().runTaskAsynchronously(SupportTickets.getInstance(), new Runnable() {
            public void run() {
                ...
                try {
                    if (connection != null) {
                        PreparedStatement preparedStatement = connection.prepareStatement(
                                "UPDATE " + SupportTicketsConfig.getTicketTable() + " SET status = ?, " +
    ...
    public List<Ticket> getTicketsClosedBy(UUID closer) {
        ...
        if (connection != null) {
            try {
                PreparedStatement statement = connection.prepareStatement(
                        "Select *, (Select uuid from " + SupportTicketsConfig.getPlayerTable() + " where player_id = t.closed_player_id) as uuidPlayerClosed from " + SupportTicketsConfig.getTicketTable() + " t"
                                + " left join " + SupportTicketsConfig.getPlayerTable() + " p on p.player_id = t.closed_player_id"
                                + " where p.uuid = ?");
            ...
            public void run() {
                ...
                try {
                    if (connection != null) {
                        ...
                        preparedStatement = connection.prepareStatement(
                                "INSERT INTO " + SupportTicketsConfig.getCommentTable() +
                                        " (ticket_id, player_id, time_stamp, comment, sender_has_noticed) VALUES" +
                                        "(?, (Select player_id from " + SupportTicketsConfig.getPlayerTable() +
                                        " where uuid = ?), ?, ?, ?)");
        ...
        SupportTickets.getInstance().getServer().getScheduler().runTaskAsynchronously(SupportTickets.getInstance(), new Runnable() {
            public void run() {
                ...
                try {
                    if (connection != null) {
                        PreparedStatement statement = connection.prepareStatement(
                                "UPDATE " + SupportTicketsConfig.getCommentTable() +
    ...
    private Ticket buildTicket(ResultSet resultSet) throws SQLException {
        ...
        PreparedStatement statement = resultSet.getStatement().getConnection().prepareStatement(
                "Select * from " + SupportTicketsConfig.getCommentTable() + " c " +
                        "left join " + SupportTicketsConfig.getPlayerTable() + " p on p.player_id = c.player_id " +
                        "where ticket_id = ? " +
