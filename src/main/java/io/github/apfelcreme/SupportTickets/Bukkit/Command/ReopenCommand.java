...
public class ReopenCommand implements SubCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (player.hasPermission("SupportTickets.reopen")) {
            if (args.length > 1) {
                if (SupportTickets.isNumeric(args[1])) {
                    ...
                    if (ticket != null) {
                        ...
                        Comment comment = new Comment(
                                ticket.getTicketId(),
                                player.getUniqueId(),
                                SupportTicketsConfig.getText("info.reopen.reopenComment")
                                        .replace("{0}", player.getName()),
                                new Date());

                        SupportTickets.getDatabaseController().saveComment(comment);

                        BungeeMessenger.sendTeamMessage(SupportTicketsConfig.getText("info.reopen.reopened")
                                .replace("{0}", ticket.getTicketId().toString()));
                        BungeeMessenger.sendMessage(ticket.getSender(),
                                SupportTicketsConfig.getText("info.reopen.yourTicketGotReopened")
                                        .replace("{0}", ticket.getTicketId().toString()));
                    } else {
